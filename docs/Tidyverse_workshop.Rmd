...
### 0.4 Overview {data-link="0.2 Preparations"}
...
[**Bonus:**]{.underline} `count()` provides a 'quick and dirty" convenience function so you can easily count e.g. the number of individuals that share the same `species` name, rather than going through a separate `group_by()` and `summarize()` call.
